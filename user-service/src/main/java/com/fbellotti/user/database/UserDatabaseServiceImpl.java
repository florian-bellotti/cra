...
public class UserDatabaseServiceImpl implements UserDatabaseService {
  ...
  public UserDatabaseService createUser(JsonObject user, Handler<AsyncResult<JsonObject>> resultHandler) {
    authProvider.insertUser(user.getString("username"), user.getString("password"), null, null, res -> {
      if (res.succeeded()) {
  ...
  @Override
  public UserDatabaseService authenticate(JsonObject user, Handler<AsyncResult<String>> resultHandler) {
    authProvider.authenticate(user, res -> {
      if (res.succeeded()) {
        User u = res.result();
        u.isAuthorised("create", canCreate -> {
          u.isAuthorised("delete", canDelete -> {
            u.isAuthorised("update", canUpdate -> {
              System.out.println("User " + u.principal().getString("_id") + " is now authenticated");
              resultHandler.handle(Future.succeededFuture());
            });
          });
        });
      } else {
        LOGGER.info("Failed to authenticate user " + user.getString("username"), res.cause());
        resultHandler.handle(Future.failedFuture(res.cause()));
      }
    });
    return this;
